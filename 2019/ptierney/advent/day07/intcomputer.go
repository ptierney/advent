...
func (c *Computer) LoadProgram(input []string) {
...
func (c *Computer) ExecuteUntilOutput() (int64, error) {
	for {
		if c.executeInstruction() == false {
			break
		}
	...
	if len(c.STD_OUTPUT_INT64) == 0 {
		return 0, fmt.Errorf("Expected output")
	}

	return c.STD_OUTPUT_INT64[0], nil
}
